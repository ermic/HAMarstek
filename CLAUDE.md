...
- **Let op: verschillende forecast-veldnamen per sensor.** De legacy sensor (`legacy_price_per_hour`) gebruikt de oude velden `datetime` (datetime-object) + `electricity_price`. De nieuwe kwartier- en uur-sensoren geven de **rauwe API-structuur** door: `start_date` (ISO-string) + `price_tax_included.amount`. De templates lezen daarom structuur-agnostisch: `start_date if defined else datetime`, en `price_tax_included.amount if defined else electricity_price`. Beide prijzen zijn integers ×1e7 (value_factor 1e-7).
